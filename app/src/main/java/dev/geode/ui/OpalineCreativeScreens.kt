...
fun OpalineCreativeRoute(
    ...
    when (destination) {
        ...
        Destination.Visuals.Customize -> OpalinePage(stringResource(R.string.oc_customize), onBack = back) { CustomizePanel(player, visualizerView) }
        Destination.Visuals.Presets -> OpalinePage(stringResource(R.string.oc_presets), onBack = back) { PresetsTreeTab(player, visualizerView) }
        Destination.Visuals.Modulation -> OpalinePage(stringResource(R.string.oc_modulation), stringResource(R.string.oc_modulation_subtitle), back) {
            CreativeScroll { ParamPage(player, visualizerView, CustomizeTab.REACTIVITY) }
        }
        Destination.Visuals.Palette -> OpalinePage(stringResource(R.string.oc_palette), onBack = back) {
            CreativeScroll { ParamPage(player, visualizerView, CustomizeTab.COLOR) }
        }
        Destination.Visuals.ShaderEditor -> OpalinePage(stringResource(R.string.oc_shader), onBack = back) {
        ...
        Destination.Visuals.Layers -> {
            ...
            LayersSheet(art, { next -> player.setOverlayOptions { next } }, watermark,
                { next -> player.setWatermarkOptions { next } }, player::pickWatermarkImage, player::clearWatermarkImage,
                lyrics, { next -> player.setOverlayLyricOptions { next } }, back)
        }
        Destination.Studio.Projects -> CreativeStudioHome(player, studio, navigator)
        ...
        Destination.Settings.Playback -> OpalinePage(stringResource(R.string.oc_playback), onBack = back) {
        ...
        Destination.Settings.ExternalAudio -> OpalinePage(stringResource(R.string.oc_external), onBack = back) {
            CreativeScroll {
                SettingsGroup(stringResource(R.string.oc_external)) { ExternalAudioSettings(player) }
                LiveInputGroup(player)
            }
        }
        Destination.Settings.AutoVisuals -> OpalinePage(stringResource(R.string.oc_auto), onBack = back) {
            CreativeScroll { SettingsGroup(stringResource(R.string.oc_auto)) { AutoVisualsGroup(player) } }
        }
        Destination.Settings.Export -> OpalinePage(stringResource(R.string.oc_export), onBack = back) {
            ExportSettingsTab(false) { navigator.open(Overlay.Export) }
        }
        Destination.Settings.Help -> OpalinePage(stringResource(R.string.oc_help), onBack = back) { HelpSettingsTab(settings) { navigator.raiseGate(Gate.TUTORIAL) } }
...
@Composable
fun OpalineExportOverlay(player: PlayerViewModel, visualizerView: VisualizerView, navigator: Navigator) {
...
fun OpalineImmersive(
    ...
    Box(Modifier.fillMaxSize().background(Color.Black)) {
        if (secondScreenName == null) {
            AndroidView(factory = { renderer }, modifier = Modifier.fillMaxSize().onGloballyPositioned { coordinates ->
                val bounds = coordinates.boundsInWindow()
                VisualizerPipCoordinator.canvasBoundsPx = Rect(
        ...
        Row(
            ...
            OpalineIconButton(Icons.Filled.SkipPrevious, stringResource(R.string.action_previous), player::previous, enabled = playback.hasMedia)
            OpalineButton(
...
@Composable
private fun CreativeVisualsHome(player: PlayerViewModel, visualizerView: VisualizerView, navigator: Navigator) {
    ...
    OpalinePage(stringResource(R.string.nav_visuals), stringResource(R.string.oc_visuals_subtitle), actions = {
        ...
        OpalinePanel(Modifier.padding(horizontal = 16.dp)) {
            Row(verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(16.dp)) {
                ...
                Column(Modifier.weight(1f)) {
                    Text(stringResource(R.string.oc_current_scene), style = MaterialTheme.typography.labelSmall, color = OpalineColors.muted)
            ...
            Row(Modifier.horizontalScroll(rememberScrollState()), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                ...
                OpalineButton(stringResource(R.string.oc_palette), { navigator.go(Destination.Visuals.Palette) }, icon = Icons.Filled.Palette)
                OpalineButton(stringResource(R.string.oc_modulation), { navigator.go(Destination.Visuals.Modulation) }, icon = Icons.Filled.GraphicEq)
        ...
        CreativeTabs(listOf(stringResource(R.string.oc_scenes), stringResource(R.string.oc_textures)), workspace, { workspace = it }, Modifier.padding(horizontal = 16.dp))
        if (workspace == 1) {
...
@Composable
private fun SceneGallery(player: PlayerViewModel, visualizerView: VisualizerView, openTextures: () -> Unit) {
    val viz by player.vizState.collectAsStateWithLifecycle()
    ...
    LazyVerticalGrid(
        GridCells.Adaptive(144.dp), Modifier.fillMaxSize(), contentPadding = PaddingValues(16.dp),
        horizontalArrangement = Arrangement.spacedBy(12.dp), verticalArrangement = Arrangement.spacedBy(12.dp),
    ) {
        items(ids, key = { it }) { id ->
            val active = id == viz.sceneId
            Column(
                Modifier.fillMaxWidth().height(138.dp).opalinePart("C01", selected = active)
                    .background(OpalineColors.deep.copy(alpha = 0.64f), RoundedCornerShape(24.dp))
                    .semantics { selected = active }.clickable(role = Role.RadioButton) { player.selectScene(id) }.padding(18.dp),
                verticalArrangement = Arrangement.SpaceBetween,
            ) {
                Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                    Icon(Icons.Filled.AutoAwesome, null, Modifier.size(28.dp), tint = if (active) OpalineColors.accent else OpalineColors.lavender)
                    if (active) Icon(Icons.Filled.Check, stringResource(R.string.oc_selected), tint = OpalineColors.accent)
                }
...
@Composable
private fun ParamPage(player: PlayerViewModel, visualizerView: VisualizerView, tab: CustomizeTab) {
    val viz by player.vizState.collectAsStateWithLifecycle()
    ParamSurface(sceneId = viz.sceneId, query = "") { CustomizeTabBody(player, visualizerView, tab) }
}

@Composable
private fun CreativeStudioHome(player: PlayerViewModel, studio: StudioViewModel, navigator: Navigator) {
    val editor by studio.editor.collectAsStateWithLifecycle()
    ...
    OpalinePage(stringResource(R.string.nav_studio), stringResource(R.string.oc_studio_subtitle), actions = {
        ...
        CreativeTabs(listOf(stringResource(R.string.oc_projects), stringResource(R.string.oc_media), stringResource(R.string.oc_takes)), tab, { tab = it }, Modifier.padding(horizontal = 16.dp))
        when (tab) {
            ...
            2 -> TakesTab(studio)
                ...
                OpalinePanel {
                    ...
                    Text(stringResource(R.string.oc_project_detail, editor.project.timeline.lanes.size, editor.project.timeline.durationMs / 1000), style = MaterialTheme.typography.bodyMedium)
                    OpalineButton(stringResource(R.string.oc_open_timeline), { navigator.go(Destination.Studio.Editor(editor.name)) }, Modifier.fillMaxWidth(), Icons.Filled.Tune)
                }
                OpalineButton(stringResource(R.string.oc_new_project), { creating = true; failed = false })
                ...
                OpalineRow(stringResource(R.string.oc_templates), stringResource(R.string.oc_templates_subtitle), { navigator.go(Destination.Studio.Templates) })
                OpalineRow(stringResource(R.string.oc_loop), stringResource(R.string.oc_loop_subtitle), { navigator.go(Destination.Studio.LoopRender) })
                OpalinePanel {
                    Text(stringResource(R.string.oc_performance), style = MaterialTheme.typography.titleMedium)
                    Text(stringResource(if (takes.recording) R.string.oc_recording else R.string.oc_performance_subtitle), style = MaterialTheme.typography.bodyMedium)
                    OpalineButton(stringResource(if (takes.recording) R.string.oc_stop_recording else R.string.oc_record), { if (takes.recording) studio.stopRecording() else studio.startRecording() }, selected = takes.recording)
                }
            }
        }
    }
        ...
        AlertDialog(
            ...
            text = {
                Column(verticalArrangement = Arrangement.spacedBy(10.dp)) {
                    OutlinedTextField(name, { name = it; failed = false }, singleLine = true, label = { Text(stringResource(R.string.oc_project_name)) })
            ...
            confirmButton = {
                OpalineButton(stringResource(R.string.oc_create), {
                    studio.createProject(name) { created ->
                        failed = !created
                        if (created) { creating = false; navigator.go(Destination.Studio.Editor(name.trim())); name = "" }
                    }
...
@Composable
private fun CreativeLookSettings(settings: SettingsViewModel, back: () -> Unit) {
    ...
    OpalinePage(stringResource(R.string.oc_look), stringResource(R.string.oc_look_subtitle), back) {
        CreativeScroll {
            SettingsGroup(stringResource(R.string.oc_motion)) {
                ...
                OpalineSlider(gui.liquidMotion, { settings.setGuiPrefs(gui.copy(liquidMotion = it)) }, range = 0f..1.5f, enabled = !gui.reducedMotion)
            }
            SettingsGroup(stringResource(R.string.oc_readability)) {
                Text(stringResource(R.string.oc_text_size, (gui.textScale * 100).toInt()))
                OpalineSlider(gui.textScale, { settings.setGuiPrefs(gui.copy(textScale = it)) }, range = GuiPrefs.TEXT_SCALE_MIN..GuiPrefs.TEXT_SCALE_MAX)
...
internal fun SettingsTabColumn(content: LazyListScope.() -> Unit) {
    LazyColumn(Modifier.fillMaxSize(), contentPadding = PaddingValues(16.dp), verticalArrangement = Arrangement.spacedBy(16.dp), content = content)
}

@Composable
internal fun SettingsGroup(title: String, modifier: Modifier = Modifier, header: (@Composable RowScope.() -> Unit)? = null, content: @Composable ColumnScope.() -> Unit) {
