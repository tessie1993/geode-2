...
@Composable
internal fun OpalinePlayerRoute(destination: Destination.Player, navigator: Navigator, player: PlayerViewModel) {
    when (destination) {
...
@Composable
internal fun OpalineMusicSheet(onDismiss: () -> Unit, content: @Composable () -> Unit) {
...
@Composable
internal fun OpalineContextSheet(onDismiss: () -> Unit, content: @Composable () -> Unit) {
...
@Composable
private fun OpalineNowPlaying(player: PlayerViewModel, navigator: Navigator) {
        ...
        LazyColumn(Modifier.fillMaxSize(), verticalArrangement = Arrangement.spacedBy(20.dp)) {
            item {
                Column(Modifier.fillMaxWidth(), horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.spacedBy(14.dp)) {
                    Box(
                        Modifier.size(252.dp).opalinePart("C20", value = if (state.isPlaying || mic.active || external.active) 1f else 0.25f)
                            .clickable { navigator.open(Overlay.Visualizer) }.padding(38.dp),
                    ) { TrackArtwork(if (external.active || mic.active) null else uri, Modifier.fillMaxSize(), corner = 999.dp) }
                    Text(
                        when { external.active -> external.nowPlaying?.title ?: stringResource(R.string.source_other_apps)
                    ...
                    Text(
                        when { external.active -> external.nowPlaying?.artist.orEmpty()
            ...
            item {
                Row(Modifier.fillMaxWidth().horizontalScroll(rememberScrollState()), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    OpalineButton(stringResource(R.string.opaline_visualize), { navigator.open(Overlay.Visualizer) }, icon = Icons.Default.AutoAwesome)
                    OpalineButton(stringResource(R.string.queue), { navigator.go(Destination.Player.Queue) }, icon = Icons.Default.QueueMusic)
                    OpalineButton(stringResource(R.string.opaline_lyrics), { navigator.go(Destination.Player.Lyrics) }, icon = Icons.Default.Lyrics)
                    OpalineButton(stringResource(R.string.opaline_sleep), { navigator.go(Destination.Player.SleepTimer) }, icon = Icons.Default.Bedtime)
                    OpalineIconButton(if (uri in favourites) Icons.Default.Favorite else Icons.Default.FavoriteBorder, stringResource(R.string.action_favourite_add), { player.toggleFavourite() }, enabled = uri != null)
                }
            }
            if (!state.hasMedia) item {
                OpalinePanel {
                    OpalineButton(stringResource(R.string.action_open_library), { navigator.show(Section.LIBRARY) }, icon = Icons.Default.LibraryMusic)
            ...
            if (queue.tracks.size > queue.index + 1) {
                ...
                itemsIndexed(queue.tracks.drop(queue.index + 1).take(3)) { index, track ->
                    OpalineRow(track.title.ifBlank { stringResource(R.string.title_untitled) }, track.artist,
...
@Composable
internal fun OpalineTransport(player: PlayerViewModel, compact: Boolean = false) {
    val state by player.uiState.collectAsStateWithLifecycle()
    ...
    OpalinePanel {
        ...
        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceEvenly, verticalAlignment = Alignment.CenterVertically) {
            OpalineIconButton(Icons.Default.Shuffle, stringResource(R.string.action_shuffle), player::toggleShuffle, Modifier.opalinePart("A03", selected = state.shuffle))
            OpalineIconButton(Icons.Default.SkipPrevious, stringResource(R.string.action_previous), player::previous, enabled = state.hasMedia)
            OpalineButton(
                stringResource(if (state.isPlaying) R.string.action_pause else R.string.action_play), player::togglePlayPause,
                icon = if (state.isPlaying) Icons.Default.Pause else Icons.Default.PlayArrow, enabled = state.hasMedia,
            )
            OpalineIconButton(Icons.Default.SkipNext, stringResource(R.string.action_next), player::next, enabled = state.hasMedia)
            OpalineIconButton(Icons.Default.Repeat, stringResource(R.string.action_repeat), player::cycleRepeatMode, Modifier.opalinePart("A03", selected = state.repeatMode != Player.REPEAT_MODE_OFF))
        }
        if (!compact) Row(Modifier.fillMaxWidth().horizontalScroll(rememberScrollState()), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            OpalineButton(stringResource(when { loop == null -> R.string.ab_loop_idle; loop?.endMs == null -> R.string.ab_loop_set_b; else -> R.string.ab_loop_looping }), player::cycleAbLoop, selected = loop != null, enabled = state.hasMedia)
            OpalineButton(stringResource(when (auto) { 1 -> R.string.auto_random; 2 -> R.string.auto_smart; 3 -> R.string.auto_sections; else -> R.string.auto_off }), player::cycleAutoMode, icon = Icons.Default.GraphicEq)
        }
    }
}

@Composable
private fun OpalineWaveform(waveform: FloatArray?, progress: Float) {
    ...
    Canvas(Modifier.fillMaxWidth().height(34.dp)) {
        ...
        repeat(bars) { i ->
            ...
            drawLine(if (i.toFloat() / bars <= progress) accent else muted,
                Offset(x, size.height * (1f - amplitude) / 2), Offset(x, size.height * (1f + amplitude) / 2),
...
internal fun OpalineInputControls(player: PlayerViewModel) {
    ...
    val micPermission = rememberLauncherForActivityResult(ActivityResultContracts.RequestPermission()) { if (it) player.setMicEnabled(true) }
    val projection = rememberLauncherForActivityResult(ActivityResultContracts.StartActivityForResult()) { result ->
        if (result.resultCode == Activity.RESULT_OK && result.data != null) PlaybackCaptureService.start(context, result.resultCode, result.data!!)
        else player.noteExternalAudioConsentDenied()
    }
    ...
    OpalinePanel {
        ...
        Row(Modifier.horizontalScroll(rememberScrollState()), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            OpalineButton(stringResource(R.string.source_live_input), {
                if (mic.active) player.setMicEnabled(false)
                else if (player.hasMicPermission()) player.setMicEnabled(true)
                else micPermission.launch(Manifest.permission.RECORD_AUDIO)
            }, icon = Icons.Default.Mic, selected = mic.active)
            if (external.supported) OpalineButton(stringResource(R.string.source_other_apps), {
                if (external.active) player.stopExternalAudio() else permissions.launch(
        ...
        if (external.refusedByApp) Text(stringResource(R.string.subtitle_capture_refused, external.refusingApp.orEmpty()), color = MaterialTheme.colorScheme.error)
    }
}

@Composable
private fun OpalineQueue(player: PlayerViewModel, navigator: Navigator) {
    val queue by player.queue.collectAsStateWithLifecycle()
    ...
    OpalinePage(stringResource(R.string.queue), onBack = { navigator.back() }, actions = {
        OpalineIconButton(Icons.Default.Add, stringResource(R.string.opaline_save_playlist), { save = !save }, enabled = queue.tracks.isNotEmpty())
    }) {
        if (save) {
            OutlinedTextField(name, { name = it }, label = { Text(stringResource(R.string.opaline_playlist_name)) }, singleLine = true, modifier = Modifier.fillMaxWidth())
            OpalineButton(stringResource(R.string.action_save), {
                library.createMusicPlaylist(name.trim(), queue.tracks.map { it.uri }); save = false
            }, enabled = name.isNotBlank() && libraryState.playlists.none { it.name.equals(name.trim(), true) })
        }
        LazyColumn(Modifier.fillMaxSize(), verticalArrangement = Arrangement.spacedBy(8.dp)) {
            if (queue.tracks.isEmpty()) item { OpalineEmptyState(stringResource(R.string.opaline_queue_empty), stringResource(R.string.opaline_queue_empty_body)) }
            itemsIndexed(queue.tracks) { index, track ->
                OpalineRow(track.title.ifBlank { stringResource(R.string.title_untitled) }, track.artist,
                    ...
                    trailing = { Row {
                        OpalineIconButton(Icons.Default.ArrowUpward, stringResource(R.string.action_up), { player.moveQueueItem(index, index - 1) }, enabled = index > 0)
                        OpalineIconButton(Icons.Default.ArrowDownward, stringResource(R.string.action_down), { player.moveQueueItem(index, index + 1) }, enabled = index < queue.tracks.lastIndex)
                        OpalineIconButton(Icons.Default.Close, stringResource(R.string.action_remove), { player.removeQueueItem(index) })
                    } },
                )
            }
        }
...
@Composable
private fun OpalineSleepTimer(player: PlayerViewModel, navigator: Navigator) {
    ...
    OpalinePage(stringResource(R.string.opaline_sleep), onBack = { navigator.back() }) {
        ...
        listOf(15, 30, 45, 60, 90).forEach { minutes ->
            OpalineRow(stringResource(R.string.opaline_minutes, minutes), onClick = { player.startSleepTimer(minutes); navigator.back() })
        }
        OpalineButton(stringResource(R.string.action_cancel), { player.cancelSleepTimer(); navigator.back() }, enabled = remaining != null)
    }
}

@Composable
private fun OpalineLyrics(player: PlayerViewModel, navigator: Navigator) {
    ...
    OpalinePage(stringResource(R.string.opaline_lyrics), state.title.orEmpty(), onBack = { navigator.back() }) {
        if (lyrics == null) OpalineEmptyState(stringResource(R.string.opaline_lyrics_empty), stringResource(R.string.opaline_lyrics_empty_body))
        else LazyColumn(Modifier.fillMaxSize(), state = listState, verticalArrangement = Arrangement.spacedBy(18.dp)) {
            itemsIndexed(lyrics!!.lines) { index, line ->
                Text(line.text, style = if (index == current) MaterialTheme.typography.headlineMedium else MaterialTheme.typography.titleLarge,
                    color = if (index == current) MaterialTheme.colorScheme.primary else MaterialTheme.colorScheme.onSurfaceVariant,
                    modifier = Modifier.fillMaxWidth().clickable(enabled = lyrics!!.synced) { player.seekToMs(line.timeMs) }.padding(vertical = 10.dp))
            }
        }
    }
}
