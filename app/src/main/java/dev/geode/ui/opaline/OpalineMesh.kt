    ...
    companion object {
        ...
        fun read(bytes: ByteArray): OpalineMesh {
            ...
            fun visit(
                index: Int,
                parent: FloatArray,
                inheritedMotion: JSONObject? = null,
                inheritedAnchor: FloatArray? = null,
                inheritedTravel: FloatArray? = null,
            ) {
                val node = nodes.getJSONObject(index)
                val transform = multiply(parent, transform(node))
                val extras = node.optJSONObject("extras")
                val ownMotion = extras?.optJSONObject("motion")
                val motion = ownMotion ?: inheritedMotion
                val anchor = if (ownMotion != null) transform else inheritedAnchor ?: transform
                val axis =
                    when (motion?.optString("axis", "x")) {
                        "y" -> 1
                        "z" -> 2
                        else -> 0
                    }
                ...
                val travel =
                    if (ownMotion != null) {
                        floatArrayOf(parent[axis * 4], parent[axis * 4 + 1], parent[axis * 4 + 2], min - translation, max - min)
                    } else {
                        inheritedTravel ?: FloatArray(5)
                    }
                if (node.has("mesh")) {
                    ...
                    for (p in 0 until primitives.length()) {
                        ...
                        pieces +=
                            Piece(
                                ...
                                morphs,
                                transform,
                                pbr.getJSONArray("baseColorFactor").floats(4),
                                pbr.optDouble("roughnessFactor", 0.2).toFloat(),
                                ...
                                extensions?.optJSONObject("KHR_materials_volume")?.optJSONArray("attenuationColor")?.floats(3)
                                ...
                                motion?.optString("kind").orEmpty(),
                                motion?.optInt("index", 0) ?: 0,
                                floatArrayOf(anchor[12], anchor[13], anchor[14]),
                                floatArrayOf(anchor[axis * 4], anchor[axis * 4 + 1], anchor[axis * 4 + 2]),
                                extras?.optString("role").orEmpty(),
                                travel,
                                extras?.optBoolean("deformable") == true,
                            )
                    }
                }
                node.optJSONArray("children")?.let { children ->
                    for (i in 0 until children.length()) visit(children.getInt(i), transform, motion, anchor, travel)
                }
            }
