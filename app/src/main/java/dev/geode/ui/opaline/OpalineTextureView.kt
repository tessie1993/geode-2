...
internal class OpalineTextureView(context: Context, private val onReady: (Boolean) -> Unit) : TextureView(context), TextureView.SurfaceTextureListener {
    ...
    fun touch(id: Long, pointer: Long, pressed: Boolean, x: Float, y: Float) {
        ...
        handler.post { renderer?.touch(id, pointer, pressed, x, y); schedule() }
    }

    fun cancel(id: Long) {
        if (!released) handler.post { renderer?.cancel(id); schedule() }
    }
    ...
    private val draw = Choreographer.FrameCallback { now ->
        pending = false
    ...
    override fun onSurfaceTextureAvailable(texture: SurfaceTexture, width: Int, height: Int) {
        ...
        handler.post {
            ...
            try {
                ...
                val attributes = intArrayOf(EGL14.EGL_RENDERABLE_TYPE, 0x40, EGL14.EGL_SURFACE_TYPE, EGL14.EGL_WINDOW_BIT,
                    EGL14.EGL_RED_SIZE, 8, EGL14.EGL_GREEN_SIZE, 8, EGL14.EGL_BLUE_SIZE, 8,
                    EGL14.EGL_ALPHA_SIZE, 8, EGL14.EGL_DEPTH_SIZE, 24, EGL14.EGL_NONE)
    ...
    private fun destroyGpu() {
        clock?.removeFrameCallback(draw)
        pending = false; lastTime = 0; ready = false; submitted = null
