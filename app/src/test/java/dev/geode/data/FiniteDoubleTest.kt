 ...
 * from `optDouble` as well as `getDouble`, whereas the stricter Maven `org.json` used by these
 * tests only lets it through `getDouble`. The tests below therefore assert the behaviour both
 * implementations share, and the helper closes the boundary on either.
 *
 * A non-finite value that reaches `SceneParams` is not transient — the renderer's interpolation
 * feeds its own output back in each frame, so one NaN poisons every interpolated field, and the
 * flash clamp cannot recover it because every comparison with a NaN is false.
 */
class FiniteDoubleTest {
    @Test
    ...
    @Test
    fun `non-finite values fall back instead of propagating`() {
    ...
    @Test
    fun `a finite value is returned unchanged`() {
        assertEquals(0.75, JSONObject("""{"x":0.75}""").finiteDouble("x", 0.0), 1e-9)
        assertEquals(-3.0, JSONObject("""{"x":-3}""").finiteDouble("x", 0.0), 1e-9)
    }

    @Test
    fun `an absent key still yields the fallback`() {
        assertEquals(1.0, JSONObject("{}").finiteDouble("missing", 1.0), 1e-9)
    }

    @Test
    fun `the required form throws on a non-finite value rather than returning one`() {
        assertThrows(org.json.JSONException::class.java) {
            JSONObject("""{"x":1e400}""").finiteDouble("x")
        }
        assertThrows(org.json.JSONException::class.java) {
            JSONObject("""{"x":"NaN"}""").finiteDouble("x")
        }
    }

    @Test
    fun `the required form still throws when the key is absent`() {
        assertThrows(org.json.JSONException::class.java) {
            JSONObject("{}").finiteDouble("missing")
        }
    }
}
