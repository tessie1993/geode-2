...
public:
    ...
    // Not RT-safe: rebuild the ramp coefficient for a new rate. Only call this on a chain not yet installed.
    void setSampleRate(float sampleRate) { coefficient_ = 1.0f - std::exp(-1.0f / (kRampSeconds * sampleRate)); }
    ...
    void process(float* interleaved, size_t frames, int channels) {
        ...
        for (size_t i = 0; i < frames; ++i) {
            ...
            // current_ only approaches target asymptotically; snap once within an LSB so a converged 0 dB
            // chain hits the early return above instead of multiplying by ~1.0f forever.
            if (std::fabs(target - current_) < kSnapEpsilon) current_ = target;
...
private:
    ...
    static constexpr float kSnapEpsilon = 1e-6f;
